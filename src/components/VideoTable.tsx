...
const VideoTable = () => {
  ...
  return (
    ...
    {!loading && <> */}
      <table>
      {vidRes.results.map((video) => (
        <tr key={video.id} className="p-2 flex flex-row mb-5 max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl">
          <td><iframe id={`${video.id}-frame`} allowFullScreen width="640" height="360" className="h-48 w-full object-cover md:h-full md:w-48 mr-2" src={video.url} /></td>
          <td>
